...
本项目是《大数据与数据工程》课程大作业，围绕 Formula 1 数据完成数据工程、统计分析、可视化和预测建模。项目从原始数据集和 API 数据出发，构建可复现的数据处理流程，生成处理后的宽表、统计分析结果、静态图表、动态视频、机器学习模型和赛季不确定性回测结果。

当前实现内容包括数据获取、数据验证、特征工程、统计分析、可视化、领奖台预测、Top 10 积分区预测、单站积分预测、F1 积分规则映射、概率校准以及 Monte Carlo 赛季不确定性模拟。最终 2026 车手总冠军和车队总冠军预测模块作为下一步应用模块保留。
...
报告用静态图表有意排除 2026 数据。2026 数据保留用于当前赛季状态分析和后续预测任务。
...
- 静态报告图表使用 2019-2025 现代历史数据和 SQLite 历史数据。
- 2026 数据不用于静态报告图表。
- 2026 数据保留用于当前赛季状态分析和后续预测。
