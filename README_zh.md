...
- 单站积分回归目标按当前 Grand Prix 正赛积分表从完赛名次重新计算：`25, 18, 15, 12, 10, 8, 6, 4, 2, 1`。
- 不直接使用 2003-2009 等历史赛季的旧积分制度作为回归目标，避免不同时代积分尺度混入训练。
- 本项目不考虑最快圈附加 1 分。
- 本项目不考虑 Sprint 冲刺赛积分。
- 默认每场比赛都有 10 个得分名额，并按正常全额正赛积分处理。
- 后续赛季积分榜、车手总冠军和车队总冠军预测应以 `rule_mapped_points` 作为主结果；连续 `predicted_points` 只作为期望积分参考。
- 数据提取、统计分析和可视化仍保留原始 `points`；预测建模阶段的赛前积分、近三场平均积分等积分型特征，按完赛名次统一换算为当前 F1 正赛积分规则。
